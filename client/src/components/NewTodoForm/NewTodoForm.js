...
const NewTodoForm = () => {
  const existErr = useSelector((state) => state.message);
  ...
  return (
    <div className={'newtodo-main'}>
      <form onSubmit={send}>
        <h3>
          <i className='far fa-calendar-plus'></i> Create New Task
        </h3>
        <input
          type={'text'}
        ...
        <small>{errState.titleErr || errState.textErr || existErr}</small>
      </form>
    </div>
  );
};
